...
#define DBG_PRT(fmt, args...)
#endif
#define DBG_ERR_PRT(fmt, args...) \
	printf("ERR:[%16s:%d] " fmt,__FUNCTION__,__LINE__,## args)
...
void fpga_selectmap_init (void)
{
	...
	ret = gpio_request(GPIO_OUT_nWE,"");
	if(ret < 0)
	{
		DBG_ERR_PRT("\n");
		return 0;
	}
	gpio_direction_output(GPIO_OUT_nWE,1);
//	gpio_set_value(GPIO_OUT_nWE,1);
}
...
int fpga_init_fn (int cookie)
{
	...
	ret = gpio_request(GPIO_IN_INIT,"");
	if(ret < 0)
	{
		DBG_ERR_PRT("\n");
		return 0;
	}
	gpio_direction_input(GPIO_IN_INIT);
	val = gpio_get_value(GPIO_IN_INIT);
	if(val)
		ret = 0;
	else
		ret = 1;
...
int fpga_done_fn (int cookie)
{
	...
	ret = gpio_request(GPIO_IN_DONE,"");
	if(ret < 0)
	{
		DBG_ERR_PRT("\n");
		return 0;
	}
	gpio_direction_input(GPIO_IN_DONE);
	val = gpio_get_value(GPIO_IN_DONE);
	if(val)
		ret = FPGA_SUCCESS;
	else
		ret = FPGA_FAIL;
...
void bitswap(unsigned char *dst,unsigned char *src,size_t len)
{
unsigned char data, org;
unsigned int i;

	printf(" BIT-SWAP: dst=%x, src=%x, len=%x\n",(unsigned)dst,(unsigned)src,len);
...
int fpga_pre_config_fn (int cookie)
{
	...
	gpio_direction_output(GPIO_OUT_nWE,0);
	gpio_set_value(GPIO_OUT_nWE,0);
	return 0;
}
